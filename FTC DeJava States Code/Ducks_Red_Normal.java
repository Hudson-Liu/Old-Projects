...
@Autonomous(name = "ducks_red", group = "auto_ducks")
public class Ducks_Red extends LinearOpMode {
    BNO055IMU imu;
    @Override
    public void runOpMode() {
        ...
        while (counter < 3) {
            if (location == duckLocations.bottom){
               if (ds.getDistance(DistanceUnit.CM) < 40 || color.getDistance(DistanceUnit.CM) < 9) {
                    counter++;
                } else {
                    counter = 0;
                } 
                ...
                if (ds.getDistance(DistanceUnit.CM) < 30 || color.getDistance(DistanceUnit.CM) < 9) {
                    counter++;
                } else {
                    counter = 0;
                }
    ...
    private duckLocations findDuck() {
        ...
        if (m == chance1) return duckLocations.top;
        if (m == chance2) return duckLocations.middle;
        return duckLocations.bottom;
        /*
